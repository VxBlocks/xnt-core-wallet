...
impl TimeLockV2 {
    ...
    /// The `type_script_hash` that `TimeLockV2` coins carried during the
    /// `UpgradeVM` era (triton-vm v3). The v4 VM upgrade re-hashed the program
    /// again, so coins committed under v3 embed this digest and must be remapped
    /// to the current `Self.hash()`. Computed by building `~/xnt-core-v3`.
    ...
    pub fn v3_type_script_hash() -> Digest {
        Digest::try_from_hex(Self::V3_TYPE_SCRIPT_HASH_HEX)
            .expect("v3 time-lock-v2 type-script-hash hex is valid")
    }

    /// Every historical (non-current) `type_script_hash` a committed
    /// `TimeLockV2` coin may carry: the pre-`UpgradeVM` legacy hash and the
    /// `UpgradeVM` (v3) hash. Both remap to the current `Self.hash()`.
...
impl TimeLockV2 {
    ...
    pub(crate) fn library_and_code_for_targets(
        ...
        // REMAP: programmatic `push` sequences for the timelock hashes the
        // program must recognize (see `push_digest_reversed`):
        //  - legacy + current + v3 TimeLock      -> extension-capable branch
        //  - current (self) + legacy + v3 TimeLockV2 -> plain branch
        ...
        let push_v3_timelock = push_digest_reversed(TimeLock::v3_type_script_hash());
        let push_legacy_timelock_v2 = push_digest_reversed(TimeLockV2::legacy_type_script_hash());
        let push_v3_timelock_v2 = push_digest_reversed(TimeLockV2::v3_type_script_hash());
        ...
        // v3 TimeLock: also match coins committed during the UpgradeVM (v3) era;
        // they belong in the extension branch (genesis targets still get +1yr).
        dup 5
        dup 5
        dup 5
        dup 5
        ...
        // _ ... [tsh] acc is_v3
        add
        // _ ... [tsh] (tsh in {legacy, current, v3} timelock)
        push 1
        swap 1
        skiz
        ...
        // v3 TimeLockV2: also match coins committed during the UpgradeVM (v3)
        // era; plain branch, never extended.
        dup 5
        dup 5
        dup 5
        dup 5
        ...
        // _ ... [tsh] acc is_v3_v2
        add
        // _ ... [tsh] (tsh in {self, legacy, v3} timelock_v2)
        swap 5
        pop 5
...
mod tests {
    ...
    fn time_lock_v2_source(target_utxo_digests: &[Digest]) {
        ...
        let v3_timelock_hash: Digest = TimeLock::v3_type_script_hash();
        let legacy_timelock_v2_hash: Digest = TimeLockV2::legacy_type_script_hash();
        let v3_timelock_v2_hash: Digest = TimeLockV2::v3_type_script_hash();
        ...
        while i < input_utxos.len() {
            ...
            while j < coins.len() {
                ...
                if coin.type_script_hash == old_timelock_hash
                    ...
                    || coin.type_script_hash == v3_timelock_hash
                {
                ...
                if coin.type_script_hash == self_digest
                    ...
                    || coin.type_script_hash == v3_timelock_v2_hash
                {
                    // extract state
                    let state: &Vec<BFieldElement> = &coin.state;
    ...
    fn legacy_tagged_timelock_coins_are_lock_enforced() {
        ...
        for tag in [
            ...
            // UpgradeVM (v3) era hashes must be lock-enforced identically.
    ...
    fn legacy_tagged_timelock_targets_apply_branch_correct_extension() {
        // The +1 year extension is granted only in the EXTENSION branch (legacy/v3
        // + current TimeLock); legacy/v3 TimeLockV2 coins sit in the PLAIN branch
        // and never get the extension, even when the UTXO is a +1 year target. The
        // existing `v2_target_*` tests only cover the *current* TimeLock hash; here
        // we pin the behaviour for BOTH the legacy (pre-UpgradeVM) and v3
        // (UpgradeVM-era) hashes.
        let release_date = Timestamp::now();

        // EXTENSION branch: legacy AND v3 TimeLock targets get the +1 year extension.
        for timelock_hash in [
            TimeLock::legacy_type_script_hash(),
            TimeLock::v3_type_script_hash(),
        ] {
            // (1) tx within the +1 year window: the extension pushes the effective
            // release past `now` -> rejected.
            let within = release_date + Timestamp::years(1) - Timestamp::millis(1);
            let w = make_witness_with_tagged_coins(timelock_hash, release_date, within);
            let prog = TimeLockV2WithTargets(targets_covering(&w));
            assert!(
                prog.run_rust(&w.standard_input(), w.nondeterminism()).is_err(),
                "old TimeLock target within +1y must be rejected (rust)"
            );
            assert!(
                prog.run_tasm(&w.standard_input(), w.nondeterminism()).is_err(),
                "old TimeLock target within +1y must be rejected (tasm)"
            );

            // (2) tx past the +1 year window -> accepted.
            let past = release_date + Timestamp::years(1) + Timestamp::days(1);
            let w = make_witness_with_tagged_coins(timelock_hash, release_date, past);
            let prog = TimeLockV2WithTargets(targets_covering(&w));
            assert!(
                prog.run_rust(&w.standard_input(), w.nondeterminism()).is_ok(),
                "old TimeLock target past +1y must be accepted (rust)"
            );
            assert!(
                prog.run_tasm(&w.standard_input(), w.nondeterminism()).is_ok(),
                "old TimeLock target past +1y must be accepted (tasm)"
            );
        }

        // (3) PLAIN branch: legacy AND v3 TimeLockV2 targets get NO extension -- a
        // coin whose release date is already in the past is spendable now despite
        // being a target (the extension would otherwise keep it locked for a year).
        for timelockv2_hash in [
            TimeLockV2::legacy_type_script_hash(),
            TimeLockV2::v3_type_script_hash(),
        ] {
            let past_release = Timestamp::now() - Timestamp::days(1);
            let w =
                make_witness_with_tagged_coins(timelockv2_hash, past_release, Timestamp::now());
            let prog = TimeLockV2WithTargets(targets_covering(&w));
            assert!(
                prog.run_rust(&w.standard_input(), w.nondeterminism()).is_ok(),
                "old TimeLockV2 target (plain branch) released must be accepted (rust)"
            );
            assert!(
                prog.run_tasm(&w.standard_input(), w.nondeterminism()).is_ok(),
                "old TimeLockV2 target (plain branch) released must be accepted (tasm)"
            );
        }
    }
    ...
    test_program_snapshot!(
        ...
        // v4 (UpgradeVMv4) program hash, after adding the v3-era TimeLock/TimeLockV2
