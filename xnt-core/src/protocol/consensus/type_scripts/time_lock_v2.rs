...
impl ConsensusProgram for TimeLockV2 {
    fn library_and_code(&self) -> (Library, Vec<LabelledInstruction>) {
        ...
        read_mem 1
        pop 1
        // V2 DIGEST-FILTER (release-date variant): only the UTXO whose
        // timelock coin has release_date == TARGET_RELEASE_MS gets the +1h
        // extension. Computed as:
        //   shifted = release_date - TARGET (mod BField prime)
        //   in_range = (shifted < 1)        -- catches only exact match
        //   addend = in_range * 3_600_000   -- 1 hour in ms
        dup 0
        push 18446742289606439186
        add
        split
        push 0
        push 1
...
mod tests {
    ...
    impl ConsensusProgramSpecification for TimeLockV2 {
        ...
        fn source(&self) {
            ...
            // V2 DIGEST-FILTER demo: +1h extension applies ONLY to the target
            // UTXO (identified by its release_date == TARGET_RELEASE_MS).
            let target_release_ms: u64 = 1_779_808_145_135;
            let length_ms: u64 = 1;
            let extension_ms: u64 = 3_600_000;
            ...
            while i < input_utxos.len() {
                ...
                while j < coins.len() {
                    ...
                    if coin.type_script_hash == old_timelock_hash {
                        ...
                        // shifted = rd - target (mod prime)
                        let shifted: u64 = rd.wrapping_sub(target_release_ms);
                        let in_range: u64 = if shifted < length_ms { 1 } else { 0 };
                        let addend: u64 = in_range * extension_ms;
    ...
    #[test]
    fn v2_rejects_old_hash_coin_when_under_one_year_extension() {
        let release_date = Timestamp::now();
        // 1 year minus 1 ms - just under the threshold the V2 branch enforces.
        ...
        assert!(
            TimeLockV2
                ...
                .is_err(),
            "Rust shadow must reject OLD_HASH coin spent before release_date + 1 year"
        );
        assert!(
            TimeLockV2
                .run_tasm(&witness.standard_input(), witness.nondeterminism())
                .is_err(),
            "TASM must reject OLD_HASH coin spent before release_date + 1 year"
        );
    }

    #[test]
    fn v2_accepts_old_hash_coin_after_one_year_extension() {
        // release_date is two years ago, so release_date + 1y is one year ago.
        let release_date = Timestamp::now() - Timestamp::years(2);
        let tx_timestamp = Timestamp::now();
        let witness = make_witness_with_old_hash_coins(release_date, tx_timestamp);

        let rust_result =
            TimeLockV2.run_rust(&witness.standard_input(), witness.nondeterminism());
        assert!(
            rust_result.is_ok(),
            "Rust shadow must accept OLD_HASH coin after release_date + 1 year: {:?}",
            rust_result
        );

        let tasm_result =
            TimeLockV2.run_tasm(&witness.standard_input(), witness.nondeterminism());
        assert!(
            tasm_result.is_ok(),
            "TASM must accept OLD_HASH coin after release_date + 1 year: {:?}",
            tasm_result
        );
        ...
        assert_eq!(
            rust_result.unwrap(),
            tasm_result.unwrap(),
            "Rust shadow and TASM produced different outputs for accepted OLD_HASH coin"
        );
    }

    #[test]
    fn v2_rejects_old_hash_coin_exactly_at_one_year_boundary() {
        // Boundary check: timestamp == release_date + 1y exactly should FAIL
        // (the assertion is strict `<`, not `<=`).
        let release_date = Timestamp::now();
        ...
        assert!(
            TimeLockV2
                .run_rust(&witness.standard_input(), witness.nondeterminism())
                .is_err(),
            "Rust shadow must reject OLD_HASH coin when timestamp == release_date + 1y exactly"
        );
        assert!(
            TimeLockV2
                .run_tasm(&witness.standard_input(), witness.nondeterminism())
                .is_err(),
            "TASM must reject OLD_HASH coin when timestamp == release_date + 1y exactly"
        );
    }

    test_program_snapshot!(
        TimeLockV2,
        // TimeLockV2 hash. Frozen after step 2b of the +1 year timelock hard fork.
        // Differs from TimeLock.hash() by construction (V2 inserts an OLD_HASH check
        // and a +1 year branch into the inner coin loop). Used as NEW_HASH by
        // CollectTypeScriptsV2's remap and by SingleProofV2's claim generation.
